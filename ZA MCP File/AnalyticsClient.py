...
class AnalyticsClient:
    ...
    class WorkspaceAPI:
        ...
        def get_query_table_details(self, view_id, config = {}):
            """Return the query table definition, including its SQL query."""
            endpoint = self.endpoint + "/querytables/" + view_id
            response = self.ac.send_api_request("GET", endpoint, config, self.request_headers)
            return response["data"]
    ...
    def regenerate_analytics_oauth_token(self):
        ...
        def refresh():
            ...
            encoded_params = urllib.parse.urlencode(oauth_params)
            req_url = self.accounts_server_url + "/oauth/v2/token"
            oauth_resp_obj = self.submit_request("POST", req_url, encoded_params)
            if oauth_resp_obj.status_code == 200:
                ...
                if "access_token" in oauth_json_resp:
                    return (
                        oauth_json_resp["access_token"],
                        oauth_json_resp.get("expires_in", 3600),
                    )
            raise ServerError(oauth_resp_obj.resp_content, True)

        self.access_token, _ = get_or_refresh_oauth_token(
            provider="zoho_analytics",
            accounts_url=self.accounts_server_url,
            client_id=self.client_id,
            refresh_token=self.refresh_token,
            refresh=refresh,
        )
